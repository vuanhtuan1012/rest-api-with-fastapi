...
# @Last Modified time: 2026-08-09 09:22:44
"""
...
@pytest.mark.usefixtures("db")
async def test_get_comments_failure(async_client: AsyncClient, endpoints: dict):
    """
    Ensures the get comments on post has existing comments returns all comments
    """
    response = await async_client.get(f"{endpoints['posts']}/999/comments")
    assert response.status_code == 404
